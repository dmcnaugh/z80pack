...
#else
	io_trap_in, 	/* port 4 */ /* SIO 2 not connected */
	io_trap_in, 	/* port 5 */ /* needed for VIO keyboard input */
#endif //IMSAI_VIO
	io_trap_in,		/* port 6 */
	io_trap_in,		/* port 7 */
 ...
 */
#ifdef ESP_PLATFORM
#define	PRINT_PATH	"/sdcard/imsai"
#else
#define PRINT_PATH
#endif //ESP_PLATFORM
#define	PRINT_FILE	PRINT_PATH "/printer.txt"
...
static BYTE clkd_in(void)
{
	...
	case 6:			/* month */
		if (clkfmt)
			...
			val = to_bcd(t->tm_mon+1);
		break;
	case 7:			/* year */
		if (clkfmt)
			val = t->tm_year%100;
		else
			val = to_bcd(t->tm_year%100);
		break;
	default:
		val = 0;
		break;
