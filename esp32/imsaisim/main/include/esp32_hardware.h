...
#define EXAMINE_NEXT(s)    (s[2] & T_EXAMINE_NEXT)
#define EXAMINE(s)         (s[2] & T_EXAMINE)
#define DEPOSIT_NEXT(s)    (s[2] & T_DEPOSIT_NEXT)
#define DEPOSIT(s)         (s[2] & T_DEPOSIT)
#define EXT_RESET(s)       (s[3] & T_EXT_RESET)
// #define RESET(s)           (s[3] & T_RESET)
#define STOP(s)            (s[3] & T_STOP)
#define RUN(s)             (s[3] & T_RUN)
#define STEP_DOWN(s)       (s[3] & T_STEP_DOWN)
#define STEP_UP(s)         (s[3] & T_STEP_UP)
#define POWER_OFF(s)       (s[3] & T_POWER_OFF)
#define POWER_ON(s)        (s[3] & T_POWER_ON)

#define NVS_POST            (nvs_settings & 0x01)
#define NVS_NO_POST         (!NVS_POST)
#define NVS_IF_STA          ((nvs_settings & 0x02) >> 1)
#define NVS_IF_AP           (!NVS_IF_STA)
#define NVS_Z80             ((nvs_settings & 0x04) >> 2)
#define NVS_I8080           (!NVS_Z80)
#define NVS_NO_UNDOC        ((nvs_settings & 0x08) >> 3)
#define NVS_UNDCO           (!NVS_NO_UNDOC)
#define NVS_4MHZ            ((nvs_settings & 0x10) >> 4)
#define NVS_2MHZ            (!NVS_4MHZ)
#define NVS_UNLIMITED       ((nvs_settings & 0x20) >> 5)
#define NVS_VIO             ((nvs_settings & 0x40) >> 6)
#define NVS_BANK_ROM        ((nvs_settings & 0x80) >> 7)
#define NVS_LOG_LEVEL       ((nvs_settings & 0x300) >> 8)
