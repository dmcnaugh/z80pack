...
usage:
				...
				printf("\t     %s\n", DISKSDIR);
#endif
				return(1); // no need to exit; just return a non-zero value
			}
...
static void save_core(void)
{
	...
	if ((fd	= open(CORE_FILE, O_WRONLY | O_CREAT, 0600)) == -1) {
...
int load_core(void)
{
	...
	if ((fd	= open(CORE_FILE, O_RDONLY)) == -1) {
		printf("can't open file %s\n", CORE_FILE);
		return(1);
	}
