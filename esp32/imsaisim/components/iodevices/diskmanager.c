...
int getDiskNumByID(const char *id) {
    ...
    LOGD(TAG, "GET ID: %s, %d", id, strlen(id));
 ...
 * 
 */
extern int DirectoryHandler(HttpdConnection_t *, void *);
extern int UploadHandler(HttpdConnection_t *, void *);
...
int LibraryHandler(HttpdConnection_t *conn, void *unused) {
    ...
    *(file_start) = '\0';
    ...
    case HTTP_GET:
        ...
        break;
    case HTTP_PUT:
        LOGI(TAG, "PUT image: image uploading.");
        return UploadHandler(conn, path);
        break;
    case HTTP_DELETE:
        if (req->len > 0) {

            i = get_body(conn, file_start, MAX_LFN - strlen(path));
            *(file_start + i) = '\0';
